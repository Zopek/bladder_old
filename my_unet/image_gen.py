...
def read_cancer_bbox(filename, image_height, image_width, label):
    ...
    bboxes_image = np.zeros((image_height, image_width), dtype=np.bool)
...
class BladderDataProvider(BaseDataProvider):
    ...
    def _process_one_channel(image, bladder_bbox):
        mean = np.mean(image)
        std = max(np.std(image), 1e-9)
        new_image = image[bladder_bbox]
        new_image = (new_image - mean) / std
    ...
    def _next_data(self):
        ...
        bladder_bbox = read_bladder_bbox(os.path.join(senior_path, dataset[self.ind][2]))
        label = dataset[self.ind][3]
        cancer_bbox_path = os.path.join('/DATA/data/yjgu/bladder/bladder_labels', dataset[self.ind][4])
