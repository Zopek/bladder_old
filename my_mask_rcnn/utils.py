...
def generate_anchors(scales, ratios, shape, feature_stride, anchor_stride):
    ...
    note: heights * widths = 1024
    '''

    # Enumerate shifts in feature space
    shifts_y = np.arange(0, shape[0], anchor_stride) * feature_stride
    '''
    shifts_y.shape = [shape[0],]
    ...
    '''
    shifts_x.shape = [shape[1], shape[1]]
    ...
    '''
    box_widths.shape = box_centers_x.shape = [65536, 3]
...
def generate_pyramid_anchors(scales, ratios, feature_shapes, feature_strides,
    ...
    '''
    anchors.shape = list(numpy.ndarray[256*256*3, 4],
                        numpy.ndarray[128*128*3, 4],
                        numpy.ndarray[64*64*3, 4],
                        numpy.ndarray[32*32*3, 4],
                        numpy.ndarray[16*16*3, 4],)
    anchor = (y1, x1, y2, x2)
    note: anchors with three ratios for every pixel in every feature_map with different scales
    '''
